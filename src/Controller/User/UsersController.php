...
#[Route(path: '/users', name: 'users'), IsGranted('ROLE_MANAGE_USERS')]
class UsersController extends AbstractController
{
     ...
     * @param User $user
     * @param Request $request
     * @param Confirm\ByToken\Handler $handler
     * @return Response
     */
    #[Route(path: '/{id}/confirm', name: '.confirm', methods: ['POST'])]
    public function confirm(User $user, Request $request, Confirm\ByToken\Handler $handler): Response
    {
        if (!$this->isCsrfTokenValid('confirm', $request->request->get('token'))) {
            return $this->redirectToRoute('users.show', ['id' => $user->getId()]);
        }

        $command = new Confirm\ByToken\Command($user->getId()->getValue());
